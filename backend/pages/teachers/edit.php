...
<div class="container" style="max-width: 1200px;" class="mx-auto">
    <div class="page-inner">
        ...
        <div class="card w-100 card-box">
            ...
            <div class="card-body">
                <form action="../../actions/teachers/update.php?id=<?= $teachers->id ?>" method="POST" enctype="multipart/form-data">

                    <div class="form-item">
                        <label for="teachers_nameinput" class="form-label">
                            <i class="bi bi-megaphone"></i> Nama
                        </label>
                        <input type="text" class="form-control" id="teachers_nameinput" name="teachers_name" value="<?= htmlspecialchars($teachers->teachers_name) ?>" required>
                    </div>

                    <div class="form-item">
                        <label for="teachers_photoinput" class="form-label">
                            <i class="bi bi-image"></i> Gambar
                        </label>
                        <img src="../../../storages/teachers/<?= htmlspecialchars($teachers->teachers_photo) ?>" alt="Gambar Galeri Saat Ini" class="img-preview-edit">
                        <input type="file" class="form-control mt-2" id="imageinput" name="teachers_photo">
                        <small class="form-text text-muted">Kosongkan jika tidak ingin mengganti gambar.</small>
                    </div>

                    <div class="form-item">
                        <label for="teachers_majorinput" class="form-label">
                            <i class="bi bi-card-text"></i> Jurusan
                        </label>
                        <input type="text" id="teachers_majorinput" class="form-control" name="teachers_major" value="<?= htmlspecialchars($teachers->teachers_major) ?>" required>
                    </div>

                    <div class="form-item">
                        <label for="jkinput" class="form-label">
                            <i class="bi bi-card-text"></i> Jenis kelamin
                        </label>
                        <select class="form-control" id="jkinput" name="jk" value="<?= htmlspecialchars($teachers->jk) ?>" required>
                            <option value="Laki-laki"<?= ($teachers->jk == 'Laki-laki') ? 'selected' : '' ?>>Laki-laki</option>
                            <option value="Perempuan"<?= ($teachers->jk == 'Perempuan') ? 'selected' : '' ?>>Perempuan</option>
                        </select>
                    </div>
