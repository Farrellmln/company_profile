<?php
    include '../../app.php';

    // cek form yang dikirm apakah dengan name ="tombol" kalau berhasil iya dan berhasil, lanjut
    if(isset($_POST['tombol'])){
        // escapeString() : untuk menghindari inputan anomali dari user
        $announcements_title = escapeString($_POST['announcements_title']);
        $date = escapeString($_POST['date']);
        $announcements = escapeString($_POST['announcements']);

        // rename file menjadi waktu standar internasional dari 1970 diacak
        $imageOld = $_FILES['announcements_image']['tmp_name'];
        $imageNew = time() . ".png";

        $storages = "../../../storages/announcements/";

        // memindahkan image lama ke penyimpanan terbaru imagwwe ne
        if(move_uploaded_file($imageOld, $storages . $imageNew)){
            $qInsert = "INSERT INTO announcements(announcements_title, announcements_image, date, announcements) VALUES ('$announcements_title', '$imageNew', '$date', '$announcements')";

            mysqli_query($connect, $qInsert) or die(mysqli_error($connect));
            echo "
                <script>
                    alert('Data Berhasil Ditambah');
                    window.location.href = '../../pages/announcements/index.php';
                </script>
            ";
        }else {
            echo "
                <script>
                    alert('Data Gagal Ditambah');
                    window.location.href = '../../pages/announcements/index.php';
                </script>
            ";
        }
    }
?>